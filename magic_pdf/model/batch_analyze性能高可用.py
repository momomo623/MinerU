...
class BatchAnalyze:
    ...
    def process_page_parallel(self, images_with_extra_info: list) -> list:
        ...
        # 打印时间统计表格
        print("=" * 80)
        print("PDF处理时间统计 (单位: 秒)")
        print("=" * 80)
        print(header_str)
        print("-" * 80)
        print(f"总耗时:\t{total_str}")
        print(f"平均每页:\t{per_page_str}")
        print("=" * 80)
    ...
    def _process_tables(self, images_with_extra_info: list, images_layout_res: list) -> float:
        ...
        max_workers = min(2, max(1, table_count))  # 最多2个线程
    ...
    def _process_ocr_recognition(self, images_layout_res: list) -> float:
        ...
        # 限制并行度为2
        max_workers = min(2, max(1, len(img_crop_lists_by_lang)))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    logger.error(f"Error in OCR recognition: {str(e)}")
